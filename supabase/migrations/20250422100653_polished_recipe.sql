/*
  # Fix Remaining uid() References in SQL - Part 3 (Fixed)

  This migration continues fixing functions,
...
DROP FUNCTION IF EXISTS get_user_bookings(uuid, text);
...
DROP FUNCTION IF EXISTS get_supplier_available_dates(uuid, date, date);
...
DROP FUNCTION IF EXISTS get_supplier_payment_summary(uuid, timestamptz, timestamptz);
