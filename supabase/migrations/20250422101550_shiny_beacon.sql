/*
  # Fix Remaining uid() References in SQL - Part 4 (Fixed)

  This migration continues fixing policies and functions,
  with proper parameter naming to avoid conflicts.
*/
...
-- Then recreate it with the correct signature and parameter names
CREATE OR REPLACE FUNCTION get_user_followed_hashtags(p_user_id uuid)
RETURNS TABLE (
  id uuid,
  user_id uuid,
  hashtag_id uuid,
...
CREATE OR REPLACE FUNCTION get_posts_from_following(p_user_id uuid)
