...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'follows') THEN
    ...
    DROP POLICY IF EXISTS "Users can create friendship requests" ON follows;
    CREATE POLICY "Users can create friendship requests" 
      ON follows FOR INSERT TO public
      WITH CHECK (auth.uid() = follower_id);
    
    -- Users can update their own friendship status
    DROP POLICY IF EXISTS "Users can update their own friendship status" ON follows;
    CREATE POLICY "Users can update their own friendship status" 
      ON follows FOR UPDATE TO public
      USING ((auth.uid() = user_id) OR (auth.uid() = friend_id));
    
    -- Users can view their own friendships
    DROP POLICY IF EXISTS "Users can view their own friendships" ON follows;
