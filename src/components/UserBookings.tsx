...
export default function UserBookings() {
  ...
  const cancelBooking = async (bookingId: string) => {
    try {
      ...
      if (data) {
        ...
        toast.textContent = 'Booking cancelled successfully!';
  ...
  const exportBookings = () => {
    // Create CSV content
    const headers = ['Date', 'Time', 'Supplier', 'Activity', 'Status', 'Price', 'Currency'];
    ...
    link.setAttribute('download', `my-bookings-${format(new Date(), 'yyyy-MM-dd')}.csv`);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span>Booked on {format(parseISO(booking.created_at), 'MMMM d, yyyy')}</span>
                    </div>
                  </div>
                </div>
