...
export default function OnboardingQuiz() {
  ...
  const calculateScores = () => {
    ...
    answers.forEach((answer, index) => {
      ...
      Object.entries(question.dimensions).forEach(([dimension, values]) => {
        newScores[dimension] = (newScores[dimension] || 0) + 
          (answer === 'A' ? values.a : values.b);
      });
    });

    Object.keys(newScores).forEach(key => {
      newScores[key] = Math.round((newScores[key] / (10 * answers.length)) * 100);
    });
      ...
      try {
        ...
        if (activities) {
          const scoredActivities = activities.map(activity => ({
            ...activity,
            score: calculateActivityScore(activity, finalScores)
          }))
          .sort((a, b) => b.score - a.score)
          .slice(0, 42);

          // Get existing user activities
          const { data: existingUserActivities, error: existingError } = await supabase
          ...
          // Filter out activities that already exist for the user
          const existingActivityIds = new Set(existingUserActivities?.map(ua => ua.activity_id) || []);
          const newActivities = scoredActivities.filter(activity => !existingActivityIds.has(activity.id));

          if (newActivities.length > 0) {
            const userActivities = newActivities.map(activity => ({
              user_id: user.id,
  ...
  const calculateActivityScore = (activity: any, scores: any) => {
    let score = 0;
    let weightSum = 0;
    ...
    Object.entries(dimensionMapping).forEach(([dimension, categories]) => {
      const dimensionScore = scores[dimension];
      const hasMatchingCategory = categories.some(category => 
        activity.category_tags?.includes(category)
      );
